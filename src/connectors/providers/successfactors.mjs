...
function parseRmkDate(value) {
  const m = String(value || '').trim().match(/^([A-Za-z]{3,9})\s+(\d{1,2}),\s*(\d{4})$/);
...
export function parseTiles(htmlText, jobBase) {
  ...
  while ((t = tileRe.exec(htmlText)) !== null) {
    ...
    const locationM = block.match(/id="[^"]*-section-location-value">([\s\S]*?)<\/div>/);
    const cityM = block.match(/id="[^"]*-section-city-value">([\s\S]*?)<\/div>/);
    const city = locationM ? clean(locationM[1]) : cityM ? clean(cityM[1]) : cityFromSlug(path, title);
    const dateM = block.match(/id="[^"]*-section-date-value">([\s\S]*?)<\/div>/);
    const parsedDate = dateM ? parseRmkDate(clean(dateM[1])) : undefined;
    const url = /^https?:\/\//i.test(path) ? path : jobBase + (path.startsWith('/') ? path : '/' + path);
    const row = { id, title, url, location: city };
    if (parsedDate !== undefined) row.postedAt = parsedDate;
    out.push(row);
  }
  return out;
}
...
// CSB posting dates are rendered in the query locale's short format: en_US
// sends M/D/YY ("6/18/26"), while en_GB sends D/M/YY ("17/07/26") and de_DE
// sends D.M.YY ("20.11.23"). The separator alone cannot distinguish the two
// slash formats, so use the locale that produced the record. Keep en_US as the
// default for callers that do not supply one, matching the provider's historic
// behavior. Anything unexpected yields undefined so a consumer sees "unknown
// date" rather than a bogus timestamp.
/** @param {unknown} raw @param {string} [locale] */
export function parseCsbDate(raw, locale = 'en_US') {
...
export function cleanCsbLocation(raw) {
  ...
  for (const p of parts) {
    const s = clean(String(p)).replace(/\bHKG\b/g, 'Hong Kong');
...
export function parseCsbJobs(json, cfg, locale) {
  ...
  for (const item of list) {
    ...
    out.push({
      ...
      postedAt: parseCsbDate(r.unifiedStandardStart, locale),
    });
  }
  return out;
}
