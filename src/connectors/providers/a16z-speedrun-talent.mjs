...
function addOptionalFilters(params, entry) {
  for (const key of ['fn', 'sen', 'emp', 'loc', 'portfolio', 'sort']) {
    const value = entry?.[key];
    if (typeof value === 'string' && value.trim()) params.set(key, value.trim());
  }
  if (entry?.remote === true || entry?.remote === 1 || entry?.remote === '1') params.set('remote', '1');
}
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    for (let page = 0; page < maxPages; page++) {
      ...
      if (q) params.set('q', q);
      addOptionalFilters(params, entry);
