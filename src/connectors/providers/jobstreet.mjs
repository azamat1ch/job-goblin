...
export function parseJobstreetItem(item, origin, fallbackCompany) {
  ...
  // JobsDB (hk/th) uses /job/<id>; Jobstreet Indonesia uses /id/job/<id>.
  const url = origin.includes('jobsdb.com') ? `${origin}/job/${jobId}` : `${origin}/id/job/${jobId}`;
  ...
  const description = [...(Array.isArray(item.bulletPoints) ? item.bulletPoints : []), item.teaser]
    .map((value) => String(value || '').trim())
    .filter(Boolean)
    .join('. ');
  const salary = String(item.salaryLabel || '').trim();
