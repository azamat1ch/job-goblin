...
function findLocationFacet(facets, requested) {
  const needle = normalizeFacetText(requested);
  const matches = [];
  function visit(nodes, inheritedParameter = '') {
    for (const node of Array.isArray(nodes) ? nodes : []) {
      const parameter = node.facetParameter || inheritedParameter;
      const label = normalizeFacetText(node.descriptor);
      if (parameter && node.id && (label === needle || label.startsWith(`${needle} `))) {
        matches.push({ parameter, id: node.id, exact: label === needle, country: /country/i.test(parameter) });
      }
      visit(node.values, parameter);
    }
  }
  visit(facets);
  return matches.sort((a, b) => Number(b.exact) - Number(a.exact) || Number(b.country) - Number(a.country))[0] || null;
}
...
export default {
   ...
   *
   * @param {{ name?: string, api?: string, careers_url?: string, max_pages?: number, location_text?: string }} entry
  ...
  async fetch(entry, ctx) {
    ...
    let appliedFacets = {};
    const makeBody = (offset) => JSON.stringify({
      limit: PAGE_SIZE,
      offset,
      searchText: typeof entry.search_text === 'string' ? entry.search_text : '',
      appliedFacets,
    });
    const sinceMs = typeof ctx?.sinceMs === 'number' ? ctx.sinceMs : null;

    let first = await fetchPageWithRetry(ctx, ep.api, { ...postOpts, body: makeBody(0) });
    if (entry.location_text) {
      const facet = findLocationFacet(first?.facets, entry.location_text);
      if (!facet) return [];
      appliedFacets = { [facet.parameter]: [facet.id] };
