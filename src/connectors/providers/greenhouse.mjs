...
const AMBIGUOUS_MULTI_LOCATION = /^multiple locations?$/i;
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    let officeMap = null;
    if (usable.some(/** @param {any} j */ j =>
      isWorkModelOnly(j.location?.name) || AMBIGUOUS_MULTI_LOCATION.test(j.location?.name || ''))) {
    ...
    return usable.map(/** @param {any} j */ (j) => {
      ...
      if (officeMap && (isWorkModelOnly(location) || AMBIGUOUS_MULTI_LOCATION.test(location))) {
